...
object PageConvertStat {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // sessionId2ActionRDD: rdd[UserVisitAction]    获取用户行为数据
  ...
  def getPageConvert(sparkSession: SparkSession,
    ...
    val pageSplitRation = new mutable.HashMap[String, Double]()
    ...
    for (pageSplit <- targetPageSplit){
      ...
      pageSplitRation.put(pageSplit, ratio)

      lastPageCount = currentPageSplitCount

    }

    val convertStr = pageSplitRation.map{
    ...
    //创建RDD
    val pageSplitRDD = sparkSession.sparkContext.parallelize(Array(pageSplitConvertRate))
